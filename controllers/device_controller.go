...
func GetFilteredDevices(c *gin.Context) {
	...
	if role != "super-admin" {
		...
		if err := config.DB.
			...
			utils.JSONError(c, http.StatusInternalServerError, "Company timezone not found")
			return
		}

		companyTimezone = company.CompanyTimezone
	}
	...
	if role != "super-admin" {
		response["company_timezone"] = companyTimezone
	}

	utils.JSONSuccess(c, "Filtered devices", response)
}
