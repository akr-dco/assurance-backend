...
func GetChainingByDeviceNew(c *gin.Context) {
	...
	for _, chain := range chainings {
		...
		// =====================
		// EXPIRE LOGIC (NON-RECURRING)
		// =====================
		if chain.FrequencyValue == 0 && chain.FrequencyUnit == "" {
			if nowLocal.After(windowEnd) {
				continue
			}
		}
