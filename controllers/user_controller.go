...
func Login(c *gin.Context) {
	...
	accessToken, expiresAt, err := utils.GenerateAccessToken(
		user.Id, user.Username, user.Email, user.Role, user.CompanyID, company.StorageType, company.ReportType,
		30*24*time.Hour)
	if err != nil {
	...
	response := gin.H{
		...
		"storage_type":  company.StorageType,
		"report_type":   company.ReportType,
		"role":          user.Role,
...
func Refresh(c *gin.Context) {
	...
	accessToken, expiresAt, err := utils.GenerateAccessToken(user.Id, user.Username, user.Email, user.Role, user.CompanyID, company.StorageType, company.ReportType, 15*time.Minute)
	if err != nil {
